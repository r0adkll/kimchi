...
class ContributesBindingAnnotation private constructor(
  ...
  companion object {
    ...
    /**
     * Find any [ContributesBinding] annotations on the given [annotated] element that are scoped to [scope]. It is
     ...
     * @return the list of [ContributesBindingAnnotation] for the given scope, or empty
     */
    fun from(annotated: KSAnnotated, scope: ClassName): List<ContributesBindingAnnotation> {
      return annotated.annotations
        ...
        .toList()
        .ifEmpty {
          throw KimchiException("Unable to find @ContributesBinding annotations for $scope", annotated)
        }
    }

    fun from(annotation: KSAnnotation): ContributesBindingAnnotation {
      val scope = annotation.argumentAt("scope", 0)
        ?.valueAsClassName
        ?: throw KimchiException("Unable to find `scope` on @ContributesBinding annotation", annotation.parent)
